...
export default function DashboardView({ 
  ...
  // Section history for backspace navigation button
  const [sectionHistory, setSectionHistory] = useState<string[]>([activeSection || 'dashboard']);
  ...
  React.useEffect(() => {
    setSectionHistory(prev => {
      ...
      if (prev[prev.length - 1] === activeSection) {
        return prev;
      }
      if (prev.length > 1 && prev[prev.length - 2] === activeSection) {
  ...
  // Reset payment states when navigating or changing sections to prevent page overlaps
  React.useEffect(() => {
  ...
  const navigateBack = React.useCallback(() => {
    ...
    if (activeSection !== 'dashboard') {
      if (sectionHistory.length > 1) {
        onSectionSelect(sectionHistory[sectionHistory.length - 2]);
      } else {
        onSectionSelect('dashboard');
      }
    }
  }, [paymentSession, activeSection, sectionHistory, onSectionSelect]);

  // Listen for physical Backspace key hook globally (when not focused on form input)
  React.useEffect(() => {
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Backspace') {
        ...
        if (
          ...
          return; // Let standard typing backspace happen
        }
  ...
  const [withdrawalNetwork, setWithdrawalNetwork] = useState<'USDT TRC20' | 'Bitcoin' | 'Ethereum' | 'USDT ERC20'>('USDT TRC20');
  const [customWithdrawalAddress, setCustomWithdrawalAddress] = useState(user.wallets?.usdtTrc20 || '');

  const handleNetworkSelect = (networkId: 'USDT TRC20' | 'Bitcoin' | 'Ethereum' | 'USDT ERC20') => {
    setWithdrawalNetwork(networkId);
    let prefill = '';
    if (networkId === 'USDT TRC20' && user.wallets?.usdtTrc20) {
      prefill = user.wallets.usdtTrc20;
    } else if (networkId === 'Bitcoin' && user.wallets?.bitcoin) {
      prefill = user.wallets.bitcoin;
    } else if (networkId === 'Ethereum' && user.wallets?.ethereum) {
      prefill = user.wallets.ethereum;
    } else if (networkId === 'USDT ERC20' && user.wallets?.usdtErc20) {
      prefill = user.wallets.usdtErc20;
    }
    setCustomWithdrawalAddress(prefill);
  };
  ...
  const [is2FAEnabled, setIs2FAEnabled] = useState(() => {
    return localStorage.getItem(`2fa_enabled_${user.username}`) === 'true';
  });
  const [twoFactorStep, setTwoFactorStep] = useState(0); // 0: inactive, 1: showing QR, 2: verify PIN
  ...
  const [storedPin, setStoredPin] = useState(() => {
    return localStorage.getItem(`with_pin_${user.username}`) || 'Not Configured';
  });
  ...
  const [activeSessions, setActiveSessions] = useState(() => {
    ...
    return [
      { id: 'sess-1', device: 'Chrome on Windows 11', location: 'London, United Kingdom (Current Session)', ip: '82.23.109.45', time: 'Active now' },
      { id: 'sess-2', device: 'Safari on iPhone 15 Pro', location: 'London, United Kingdom', ip: '82.23.109.81', time: '2 hours ago' },
      { id: 'sess-3', device: 'Firefox on macOS Monterey', location: 'New York, United States', ip: '142.250.72.10', time: 'Yesterday, 14:15' }
    ];
  });
  ...
  // Dynamic Referrals History State
  const [referralHistory, setReferralHistory] = useState<any[]>(() => {
    const saved = localStorage.getItem(`referral_history_${user.username}`);
    if (saved) {
      try {
    ...
    // New users start from scratch with no reference history ($0.00 total)
    return [];
  });

  const handleClearReferralLogs = () => {
    setReferralHistory([]);
    localStorage.setItem(`referral_history_${user.username}`, JSON.stringify([]));
  };

  const handleResetReferralLogs = () => {
    const defaultData = [
      { id: "REF-8372", username: "cryptoking_9", date: "2026-05-28", level: "Tier 1 (Direct)", depositSum: 500.00, commission: 35.00, status: "Paid" },
      { id: "REF-5091", username: "blockchain_dev", date: "2026-05-29", level: "Tier 1 (Direct)", depositSum: 1500.00, commission: 105.00, status: "Paid" },
      { id: "REF-2290", username: "eth_holder_x", date: "2026-05-30", level: "Tier 2 (Indirect)", depositSum: 1000.00, commission: 30.00, status: "Paid" }
    ];
    setReferralHistory(defaultData);
    localStorage.setItem(`referral_history_${user.username}`, JSON.stringify(defaultData));
  };

  const handleAddReferralMock = () => {
    const listNames = ['alpha_trader', 'rich_whale', 'bull_runner', 'moon_hodler', 'defi_architect'];
    const randomName = listNames[Math.floor(Math.random() * listNames.length)] + '_' + Math.floor(Math.random() * 90 + 10);
    const mockDeposit = Math.floor(Math.random() * 15 * 100) + 100; // $100 - $1600
    const mockComm = mockDeposit * 0.07;
    const newRef = {
      id: `REF-${Math.floor(Math.random() * 8999 + 1000)}`,
      username: randomName,
      date: new Date().toISOString().split('T')[0],
      level: "Tier 1 (Direct)",
      depositSum: mockDeposit,
      commission: mockComm,
      status: "Paid"
    };
    const updated = [newRef, ...referralHistory];
    setReferralHistory(updated);
    localStorage.setItem(`referral_history_${user.username}`, JSON.stringify(updated));
  };
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    if (profilePhoto && profilePhoto.startsWith('data:')) {
      ...
      try {
        const storageRef = ref(storage, `users/${uid}/profilePhoto_${Date.now()}`);
        console.log(`[Firebase Storage] Uploading new profile photo to bucket for user: ${uid}...`);
        const uploadResult = await uploadString(storageRef, profilePhoto, 'data_url');
        photoUrl = await getDownloadURL(uploadResult.ref);
        setProfilePhoto(photoUrl);
        console.log(`[Firebase Storage] Profile photo successfully uploaded to URL: "${photoUrl}"`);
      } catch (err) {
        console.error('[Firebase Storage] Failed to upload profile photo:', err);
        alert('An error occurred while uploading your profile photo to Firebase Storage. Saving profile with local representation instead.');
      }
    }
  ...
  const handlePasswordChangeSubmit = (e: React.FormEvent) => {
    ...
    // Success Simulation
    setPwSuccessMsg('Success! Your account password has been hardened and updated.');
    setOldPassword('');
    setNewPassword('');
    setConfirmPassword('');
    setTimeout(() => {
  ...
  const handleSetWithdrawalPinSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setPinSuccessMsg('');
    setPinErrorMsg('');

    const isNumeric = /^\d+$/.test(withdrawalPin);
    if (!isNumeric || withdrawalPin.length !== 4) {
      setPinErrorMsg('Withdrawal PIN must be exactly 4 numeric digits.');
      return;
    }

    if (withdrawalPin !== confirmWithdrawalPin) {
      setPinErrorMsg('Confirm PIN does not match.');
      return;
    }

    localStorage.setItem(`with_pin_${user.username}`, withdrawalPin);
    setStoredPin(withdrawalPin);
    setPinSuccessMsg('Success! Secure Transaction withdrawal PIN is now active.');
    setWithdrawalPin('');
  ...
  const handleToggle2FAStatus = () => {
    if (is2FAEnabled) {
      ...
      localStorage.setItem(`2fa_enabled_${user.username}`, 'false');
      setSecuritySuccess('Two-Factor Authentication has been successfully disabled.');
  ...
  const handleVerify2FACode = (e: React.FormEvent) => {
    ...
    if (enteredCode.trim().length !== 6 || !/^\d+$/.test(enteredCode)) {
      setSecurityError('Verification code must be exactly 6 numeric digits.');
      return;
    }

    setIs2FAEnabled(true);
    localStorage.setItem(`2fa_enabled_${user.username}`, 'true');
    setTwoFactorStep(0);
    setEnteredCode('');
    setSecuritySuccess('Congratulation! Authenticator 2FA is now fully active to protect all withdrawals.');
    setTimeout(() => setSecuritySuccess(''), 4000);
  };

  const handleRevokeSessionItem = (sessionId: string) => {
    const updated = activeSessions.filter((s: any) => s.id !== sessionId);
    setActiveSessions(updated);
    localStorage.setItem(`sessions_${user.username}`, JSON.stringify(updated));
    setSecuritySuccess('Security session successfully revoked. Authorized device is now logged out.');
    setTimeout(() => setSecuritySuccess(''), 4000);
  };
  ...
  const handleConfirmPayment = async (simulateInstantApprove = false) => {
    ...
    let proofUrl = paymentProofFile || '';
    if (paymentProofFile && paymentProofFile.startsWith('data:')) {
      try {
        const storageRef = ref(storage, `payments/${uid}/paymentProof_${timestamp}`);
        console.log(`[Firebase Storage] Uploading payment proof image for user: ${uid}...`);
        const uploadResult = await uploadString(storageRef, paymentProofFile, 'data_url');
        proofUrl = await getDownloadURL(uploadResult.ref);
        setPaymentProofFile(proofUrl);
        console.log(`[Firebase Storage] Payment proof uploaded successfully to: "${proofUrl}"`);
      } catch (err) {
        console.error('[Firebase Storage] Failed to upload payment proof to Storage:', err);
      }
    }
  ...
  const handleWithdrawalSubmit = async (e: React.FormEvent) => {
    ...
    if (amount < 1000) {
      alert("Minimum payout withdrawal limit is $1,000.00 USD.");
      return;
    }
  ...
  return (
    <div className="flex-1 bg-slate-100 flex flex-col overflow-y-auto overflow-x-hidden w-full relative">
        ...
        <div className="flex items-center gap-3">
          {(activeSection !== 'dashboard' || paymentSession) && (
            <button
              ...
              className="mr-2 px-3.5 py-2 bg-slate-900 hover:bg-[#00c2b2] text-white hover:text-slate-950 rounded-xl transition-all duration-200 cursor-pointer flex items-center justify-center gap-1.5 text-[10px] md:text-xs font-black uppercase tracking-wider font-sans group animate-in slide-in-from-left-3 hover:shadow-lg hover:shadow-[#00c2b2]/20"
              title="Return to Previous Page [Backspace]"
            >
              <ArrowLeft size={14} className="transition-transform group-hover:-translate-x-0.5 stroke-[3]" />
              <span className="flex items-center gap-1.5 leading-none font-sans font-black">
                Go Back
                <kbd className="hidden md:inline-flex items-center justify-center px-1.5 py-0.5 text-[9px] font-mono font-bold bg-slate-800 text-slate-400 group-hover:bg-slate-950/20 group-hover:text-slate-950 border border-slate-700/60 group-hover:border-slate-950/20 rounded shadow-xs leading-none">⌫</kbd>
              </span>
            </button>
          )}
          {onToggleSidebar && (
          ...
          >
            ...
            <div className="w-8.5 h-8.5 rounded-full border border-slate-200 overflow-hidden shrink-0 relative shadow-inner bg-slate-100">
              <SafeImage 
                ...
                fallbackSrc="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?q=80&w=150&auto=format&fit=crop"
              />
      ...
      <div className="p-6 flex flex-col gap-6 max-w-7xl w-full mx-auto">
              ...
              <div className="p-6 md:p-8">
                <form onSubmit={(e) => handleFundingDeposit(e, false)} className="space-y-6">
                  ...
                  <div className="bg-slate-50 rounded-xl p-4 border border-slate-200/80">
                    <label className="block text-xs font-black uppercase text-slate-500 tracking-wider mb-3">
                      🔒 Official Corporate Receiving Wallet Addresses
                    </label>
                    <div className="space-y-3">
                      {[
                        { id: 'usdt_trc25', key: 'usdt_trc20', label: 'USDT (TRC20)', name: 'Tether USD TRC-20', net: 'TRON (TRC20)' },
                        { id: 'btc', key: 'btc', label: 'Bitcoin (BTC)', name: 'Bitcoin (BTC)', net: 'Bitcoin Mainnet' },
                        { id: 'eth', key: 'eth', label: 'Ethereum (ETH)', name: 'Ethereum (ETH)', net: 'Ethereum (ERC20)' },
                        { id: 'usdt_erc20', key: 'usdt_erc20', label: 'USDT (ERC20)', name: 'Tether USD ERC-20', net: 'Ethereum (ERC20)' }
                      ].map((wallet) => {
                        const addr = COMPANY_WALLET_ADDRESSES[wallet.key]?.address || '';
                        const isCopied = copiedAddressId === wallet.id;
                        const isSelected = selectedFundingMethod === wallet.key;
                        ...
                        return (
                          <div 
                            ...
                            className={`p-3 rounded-lg border transition-all ${
                              isSelected 
                                ? 'bg-emerald-50/40 border-[#00c2b2] shadow-xs' 
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                              <div>
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  <span className="text-xs font-extrabold text-[#0a1626]">{wallet.name}</span>
                                  <span className="text-[9px] font-bold bg-[#00c2b2]/10 text-[#00a093] px-1.5 py-0.5 rounded uppercase font-mono tracking-wider">{wallet.net}</span>
                                  {isSelected && (
                                    <span className="text-[8px] font-black bg-[#9333ea]/10 text-[#9333ea] px-1.5 py-0.5 rounded uppercase tracking-wider">Currently Selected</span>
                                  )}
                                </div>
                                <div className="text-[10px] font-mono text-slate-600 break-all select-all pt-1 font-semibold">{addr}</div>
                              </div>
                              <button
                                type="button"
                                onClick={() => {
                                  navigator.clipboard.writeText(addr);
                                  setCopiedAddressId(wallet.id);
                                  setTimeout(() => setCopiedAddressId(null), 2500);
                                }}
                                className={`px-3 py-1.5 rounded-md font-black text-[9px] uppercase tracking-wider transition-all flex items-center gap-1 shrink-0 cursor-pointer ${
                                  isCopied 
                                    ? 'bg-emerald-500 hover:bg-emerald-600 text-white' 
                                    : 'bg-[#0a1626] hover:bg-[#122845] text-white animate-pulse'
                                }`}
                              >
                                {isCopied ? (
                                  <>
                ...
                <form onSubmit={handleWithdrawalSubmit} className="space-y-6">
                  <div>
                    ...
                    <div className="text-[11px] text-slate-400 font-semibold mt-1.5 flex justify-between">
                      <span>Minimum payout: $1,000.00</span>
                  ...
                  <div>
                    <label className="block text-xs font-black uppercase text-slate-500 tracking-wider mb-2">
                      Select Crypto Network for Payout
                    </label>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                      {[
                        { id: 'USDT TRC20', label: 'USDT (TRC20)', walletKey: 'usdtTrc20', networkName: 'TRON Network (TRC-20)' },
                        { id: 'Bitcoin', label: 'Bitcoin (BTC)', walletKey: 'bitcoin', networkName: 'Bitcoin Native Network' },
                        { id: 'Ethereum', label: 'Ethereum (ETH)', walletKey: 'ethereum', networkName: 'Ethereum Network (ERC-20)' },
                        { id: 'USDT ERC20', label: 'USDT (ERC20)', walletKey: 'usdtErc20', networkName: 'Tether Ledger (ERC-20)' },
                      ].map((net) => {
                        const isSelected = withdrawalNetwork === net.id;
                        return (
                          <button
                            key={net.id}
                            type="button"
                            onClick={() => handleNetworkSelect(net.id as any)}
                            className={`p-3 rounded-xl border text-left transition-all cursor-pointer flex items-center justify-between ${
                              isSelected 
                                ? 'border-violet-600 bg-violet-50/50 text-violet-950 ring-2 ring-violet-600/10' 
                                : 'border-slate-200 bg-white hover:border-slate-300 text-slate-800'
                            }`}
                          >
                            <div className="flex flex-col">
                              <span className="text-xs font-extrabold">{net.label}</span>
                              <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wide mt-0.5">{net.networkName}</span>
                            </div>
                            {isSelected ? (
                    ...
                    <label className="block text-xs font-black uppercase text-slate-500 tracking-wider mb-2">
                      Recipient {withdrawalNetwork} Wallet Address
                    </label>
                    <input 
                      type="text"
                      required
                      ...
                      placeholder={
                        withdrawalNetwork === 'USDT TRC20' ? 'TFc1S7BvXU...' :
                        withdrawalNetwork === 'Bitcoin' ? '1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa...' :
                        withdrawalNetwork === 'Ethereum' ? '0x71C765...' : '0x71C765...'
                      }
                    />
                    <p className="text-[10px] text-slate-400 font-medium mt-1.5 flex items-center gap-1.5">
                      <span className="text-amber-500">⚠️</span> Please ensure your destination recipient address belongs to the <strong className="text-slate-650 uppercase underline decoration-dotted">{withdrawalNetwork}</strong> network.
                    </p>
                  </div>
        ...
        {/* ===== REFERRAL AFFILIATE LINKS VIEW ===== */}
        {(activeSection === 'ref-links' || activeSection === 'tell-a-friend') && (
          <div className="w-full max-w-4xl mx-auto p-4 md:p-8 animate-in fade-in duration-300">
            <div className="bg-gradient-to-br from-[#0a1626] to-[#040e1b] rounded-3xl p-6 md:p-8 border border-slate-800 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6 text-white mb-8">
              <div>
                <span className="bg-[#00c2b2]/10 text-[#00c2b2] text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full border border-[#00c2b2]/20 mb-3 inline-block font-sans">
                  Affiliate Link Generator
                </span>
                <h2 className="text-2xl md:text-3xl font-black uppercase font-display tracking-tight text-white mb-2 leading-none">
                  Share & Earn Passive ROI
                </h2>
                <p className="text-slate-400 text-xs font-semibold max-w-xl leading-relaxed font-sans mt-2">
                  Unlock tier-based bonuses by recruiting members to Chibuike Secure Portal. Copy and share your unique, functionally active reference links below.
                </p>
              </div>
              <div className="w-16 h-16 rounded-2xl bg-[#00c2b2]/10 flex items-center justify-center text-[#00c2b2] border border-[#00c2b2]/20 shrink-0 self-start md:self-center">
                <Share2 size={28} />
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              {/* Box 1: Global Production Link */}
              <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-sm flex flex-col justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse"></span>
                    <span className="text-xs font-black uppercase text-slate-500 tracking-wider">Corporate Production URL</span>
                  </div>
                  <p className="text-slate-400 text-[11px] leading-relaxed mb-3 font-sans">
                    Use this official branded link to onboard global investors through our main portal.
                  </p>
                  <div className="bg-slate-50 px-3 py-2.5 rounded-xl border border-slate-100 font-mono text-[10px] sm:text-xs text-slate-700 font-bold select-all break-all leading-normal">
                    {`https://chibuike.com/?ref=${user.username}`}
                  </div>
                </div>
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(`https://chibuike.com/?ref=${user.username}`);
                    setCopiedRefId('prod');
                    setTimeout(() => setCopiedRefId(null), 2500);
                  }}
                  className={`w-full py-2.5 rounded-xl font-black text-xs uppercase tracking-wider transition-all cursor-pointer flex items-center justify-center gap-2 ${
                    copiedRefId === 'prod'
                      ? 'bg-emerald-500 hover:bg-emerald-600 text-white font-sans'
                      : 'bg-slate-900 hover:bg-slate-800 text-white shadow-sm font-sans'
                  }`}
                >
                  {copiedRefId === 'prod' ? (
                    <>
                      <Check size={14} className="stroke-[3]" /> Instantly Copied!
                    </>
                  ) : (
                    <>
                      <Copy size={12} /> Copy Branded Link
                    </>
                  )}
                </button>
              </div>

              {/* Box 2: Secure Dev Sandbox Tester Link */}
              <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-sm flex flex-col justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="w-2.5 h-2.5 rounded-full bg-violet-500 animate-pulse"></span>
                    <span className="text-xs font-black uppercase text-slate-500 tracking-wider">Dev Sandbox Preview Link</span>
                  </div>
                  <p className="text-slate-400 text-[11px] leading-relaxed mb-3 font-sans">
                    Perfect for testing registration behaviors within this active AI Studio preview container.
                  </p>
                  <div className="bg-slate-50 px-3 py-2.5 rounded-xl border border-slate-100 font-mono text-[10px] sm:text-xs text-slate-700 font-bold select-all break-all leading-normal">
                ...
                <button
                  ...
                  className={`w-full py-2.5 rounded-xl font-black text-xs uppercase tracking-wider transition-all cursor-pointer flex items-center justify-center gap-2 ${
                    copiedRefId === 'dev'
                      ? 'bg-emerald-500 hover:bg-emerald-600 text-white font-sans'
                      : 'bg-violet-600 hover:bg-violet-700 text-white shadow-sm font-sans'
                  }`}
                >
                  {copiedRefId === 'dev' ? (
                    <>
                    ...
                    <>
                      <Copy size={12} /> Copy Dev Preview URL
                    </>
                  )}
                </button>
              </div>
            ...
            {/* Commission structures & info */}
            <div className="bg-white border border-slate-200/80 rounded-2xl p-5 md:p-6 hover:shadow-xs transition-all mb-8">
              <h3 className="text-xs font-black uppercase text-slate-950 tracking-wider mb-4 flex items-center gap-2 font-sans">
                💰 Multi-Tier Referral Incentive Model
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="p-4 bg-slate-50 border border-slate-100 rounded-xl text-center">
                  <div className="text-2xl font-black font-display text-emerald-600">7.00%</div>
                  <div className="text-[10px] font-black uppercase text-slate-500 tracking-wider mt-1 font-sans">Tier 1 Partners</div>
                  <div className="text-[9px] text-slate-400 font-bold uppercase mt-0.5 font-sans">Onboarded directly by you</div>
                </div>
                <div className="p-4 bg-slate-50 border border-slate-100 rounded-xl text-center">
                  <div className="text-2xl font-black font-display text-violet-600">3.00%</div>
                  <div className="text-[10px] font-black uppercase text-slate-500 tracking-wider mt-1 font-sans">Tier 2 Partners</div>
                  <div className="text-[9px] text-slate-400 font-bold uppercase mt-0.5 font-sans">Invited by your Tier 1 peers</div>
                </div>
                <div className="p-4 bg-slate-50 border border-slate-100 rounded-xl text-center">
                  <div className="text-2xl font-black font-display text-slate-700">1.00%</div>
                  <div className="text-[10px] font-black uppercase text-slate-500 tracking-wider mt-1 font-sans">Tier 3 Partners</div>
                  <div className="text-[9px] text-slate-400 font-bold uppercase mt-0.5 font-sans">Sub-tier downline referrals</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200/80 rounded-2xl p-6 text-center">
              <h4 className="text-[#0a1626] text-xs font-black uppercase tracking-wider mb-3 font-sans">
                Broadcast Your Link Seamlessly
              </h4>
              <div className="flex flex-wrap items-center justify-center gap-3">
                <a
                  href={`https://t.me/share/url?url=${encodeURIComponent(window.location.origin + '/?ref=' + user.username)}&text=${encodeURIComponent('Join Chibuike secure capital portal and initiate smart micro profits!')}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-4 py-2 bg-[#229ED9] text-white rounded-lg font-bold text-[10px] uppercase tracking-wider hover:bg-[#1f8fc4] transition-all cursor-pointer inline-flex items-center gap-1.5 font-sans"
                >
                  Share on Telegram
                </a>
                <a
                  href={`https://api.whatsapp.com/send?text=${encodeURIComponent('Join Chibuike secure capital portal here: ' + window.location.origin + '/?ref=' + user.username)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-4 py-2 bg-[#25D366] text-white rounded-lg font-bold text-[10px] uppercase tracking-wider hover:bg-[#20ba59] transition-all cursor-pointer inline-flex items-center gap-1.5 font-sans"
                >
                  Share on WhatsApp
                </a>
                <a
                  href={`mailto:?subject=${encodeURIComponent('Chibuike Safe Capital Invite')}&body=${encodeURIComponent('Join Chibuike secure capital portal and track high-tier ledger profits. Register using this affiliate link: ' + window.location.origin + '/?ref=' + user.username)}`}
                  className="px-4 py-2 bg-slate-800 text-white rounded-lg font-bold text-[10px] uppercase tracking-wider hover:bg-slate-900 transition-all cursor-pointer inline-flex items-center gap-1.5 font-sans"
                >
                  Invite via Mail
                </a>
              </div>
        ...
        {/* ===== REFERRALS HISTORY & PERFORMANCE VIEW ===== */}
        {activeSection === 'referrals' && (
          <div className="w-full p-4 md:p-8 animate-in fade-in duration-300">
            <h3 className="text-xl font-black text-slate-805 uppercase tracking-widest mb-6 font-display flex items-center gap-2">
              <Users className="text-[#00c2b2]" size={20} /> Referrals Performance & logs
            </h3>

            {/* Performance Summary metric cards */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              <div className="bg-white rounded-xl p-4.5 border border-slate-200/80 shadow-sm flex items-center justify-between">
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Total Referrals</span>
                  <span className="text-2xl font-black font-display text-slate-805 leading-none">{referralHistory.length}</span>
                  <span className="text-[9px] text-[#00c2b2] font-black uppercase mt-1 font-sans">
                    💡 Network Nodes
                  </span>
                </div>
              ...
              <div className="bg-white rounded-xl p-4.5 border border-slate-200/80 shadow-sm flex items-center justify-between">
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-sans">Active Depositors</span>
                  <span className="text-2xl font-black font-display text-slate-805 leading-none">
                    {referralHistory.filter(r => r.depositSum > 0).length}
                  </span>
                  <span className="text-[9px] text-slate-450 mt-1 font-bold uppercase font-sans">Funded downlines</span>
                </div>
              ...
              <div className="bg-white rounded-xl p-4.5 border border-slate-200/80 shadow-sm flex items-center justify-between">
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-sans">Total Commission</span>
                  <span className="text-2xl font-black font-display text-emerald-600 leading-none">
                    ${referralHistory.reduce((sum, r) => sum + r.commission, 0).toFixed(2)}
                  </span>
                  <span className="text-[9px] text-slate-450 mt-1 font-bold uppercase font-sans">Credited Profits</span>
                </div>
                <div className="w-10 h-10 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center">
                  <Coins size={18} />
                </div>
            ...
            {/* Sandbox Operations Bar */}
            <div className="bg-violet-50/50 rounded-xl p-4 border border-violet-100 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-1.5">
                <span className="text-[9px] font-black bg-violet-600 text-white px-2 py-0.5 rounded uppercase tracking-wider font-sans">
                  Evaluation Panel
                </span>
                <span className="text-[11px] text-violet-950 font-bold font-sans">Toggle live states to verify the "no history yet" layout.</span>
              </div>
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={handleAddReferralMock}
                  className="px-3 py-1.5 bg-violet-600 text-[9px] text-white font-black uppercase tracking-wider rounded-lg shadow-sm hover:bg-violet-700 cursor-pointer"
                >
                  ⚡ Register Peer Sponsor Node
                </button>
                {referralHistory.length > 0 ? (
                  <button
                    onClick={handleClearReferralLogs}
                    className="px-3 py-1.5 bg-rose-600 text-[9px] text-white font-black uppercase tracking-wider rounded-lg shadow-sm hover:bg-rose-700 cursor-pointer"
                  >
                    🗑️ Reset to Empty State
                  </button>
                ) : (
                  <button
                    onClick={handleResetReferralLogs}
                    className="px-3 py-1.5 bg-emerald-600 text-[9px] text-white font-black uppercase tracking-wider rounded-lg shadow-sm hover:bg-emerald-700 cursor-pointer"
                  >
                    ♻️ Pre-populate Demo Logs
                  </button>
                )}
              </div>
            </div>

            {/* CONDITIONAL STATEMENT CHECK: current latest history or no history yet */}
            {referralHistory.length === 0 ? (
              <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center flex flex-col items-center gap-4 animate-in fade-in max-w-lg mx-auto">
                ...
                <p className="text-xs text-slate-400 font-normal leading-relaxed max-w-sm font-sans">
                  You do not have any registered sub-affiliates or downline referrals at this stage. Generate your unique affiliate links can start inviting others to earn.
                </p>
                <div className="flex gap-3 justify-center mt-2">
                  <button 
                    onClick={() => onSectionSelect('ref-links')}
                    className="px-5 py-2.5 bg-[#00c2b2] text-[10px] font-black text-white uppercase tracking-wider rounded-lg shadow-sm cursor-pointer hover:bg-[#00a395]"
                  >
              ...
              // Total Referral History List Table
              <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-100 text-[10px] font-black uppercase text-slate-500 tracking-wider">
                        ...
                        <th className="py-4 px-3 font-sans">Affiliation Tier</th>
                        <th className="py-4 px-3 text-right">Downline Deposit</th>
                        <th className="py-4 px-3 text-right">Commission (7% / 3%)</th>
                        <th className="py-4 px-6 text-right font-sans">Registration Date</th>
                        <th className="py-4 px-6 text-center font-sans">Status</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50 text-xs text-slate-600 font-medium font-mono">
                      {referralHistory.map((refItem) => (
                        <tr key={refItem.id} className="hover:bg-slate-50/50">
                          <td className="py-4 px-6 font-bold text-slate-900">{refItem.id}</td>
                          <td className="py-4 px-3">
                            <span className="font-bold text-slate-850 font-sans text-xs">@{refItem.username}</span>
                          </td>
                          <td className="py-4 px-3 font-sans font-semibold">
                            <span className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider ${
                              refItem.level.includes('Tier 1') ? 'bg-emerald-50 text-emerald-600' : 'bg-violet-50 text-violet-600'
                            }`}>
                              {refItem.level}
                            </span>
                          </td>
                          <td className="py-4 px-3 text-right text-slate-805 font-bold">${refItem.depositSum.toFixed(2)}</td>
                          <td className="py-4 px-3 text-right font-black text-emerald-600">+${refItem.commission.toFixed(2)}</td>
                          <td className="py-4 px-6 text-right text-[10px] text-slate-400">
                            {refItem.date}
                          </td>
                          <td className="py-4 px-6 text-center font-sans font-bold">
                            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest bg-emerald-100 text-emerald-700">
                              {refItem.status}
                            </span>
                          </td>
        ...
        {activeSection === 'security' && (
          <div className="bg-white rounded-2xl border border-slate-200/80 p-6 md:p-8 shadow-sm max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="border-b border-slate-100 pb-5 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h2 className="text-xl font-black font-display text-slate-800 tracking-tight uppercase flex items-center gap-2">
                  <Lock className="text-[#00c2b2]" size={22} /> Account Security Center
                </h2>
                <p className="text-xs text-slate-400 mt-1">Harden your access credentials, configure multi-factor protections, and manage active ledger authorizations.</p>
              </div>
              <div className="flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-700 rounded-full border border-emerald-100 text-[10px] font-black uppercase tracking-widest self-start font-sans">
                <ShieldCheck size={12} /> Portal Encrypted
              </div>
            </div>

            {/* Main Notifications banners */}
            {securitySuccess && (
              <div className="mb-6 p-4 bg-emerald-50 border border-emerald-100 text-emerald-800 text-xs font-semibold rounded-xl flex items-start gap-2.5 animate-in fade-in zoom-in-95">
                <span className="text-emerald-500 text-sm">✅</span>
                <p>{securitySuccess}</p>
              </div>
            )}
            {securityError && (
              <div className="mb-6 p-4 bg-rose-50 border border-rose-100 text-rose-800 text-xs font-semibold rounded-xl flex items-start gap-2.5 animate-in fade-in zoom-in-95">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              {/* Left Side: 2FA & Secure PIN */}
              <div className="lg:col-span-7 space-y-8">
                
                {/* 1. Two-Factor Authentication (2FA) */}
                <div className="p-5 bg-slate-50/50 rounded-2xl border border-slate-150 space-y-4">
                  <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                    <div className="flex items-center gap-2">
                      <Smartphone size={18} className="text-[#00c2b2]" />
                      <h3 className="text-xs font-black uppercase text-slate-700 tracking-wider">Two-Factor Authentication (2FA)</h3>
                    </div>
                    <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider ${
                      is2FAEnabled ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'
                    }`}>
                  ...
                  <p className="text-slate-400 text-[11px] leading-relaxed">
                    Prevent unauthorized login or withdrawal requests. Secure your ledger by binding a standard Google Authenticator scan keys to your mobile phone.
                  </p>
                  ...
                  {twoFactorStep === 0 ? (
                    <div className="pt-2">
                      <button
                        ...
                        className={`px-4 py-2 text-[10px] font-black uppercase tracking-wider rounded-xl transition-all cursor-pointer flex items-center gap-2 ${
                          is2FAEnabled 
                            ? 'bg-rose-500 hover:bg-rose-600 text-white' 
                            : 'bg-[#9333ea] hover:bg-purple-700 text-white'
                        }`}
                      >
                        {is2FAEnabled ? 'Disable Authenticator 2FA' : 'Enable Authenticator 2FA'}
                      </button>
                    </div>
                  ) : (
                    <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-4 animate-in fade-in zoom-in-95">
                      <div className="flex flex-col sm:flex-row items-center gap-4">
                        {/* High Fidelity Mock QR Code */}
                        <div className="w-24 h-24 bg-slate-100 border border-slate-200 rounded-lg p-2.5 flex flex-col justify-between shrink-0 relative overflow-hidden flex items-center justify-center">
                          <div className="grid grid-cols-4 gap-1 w-full h-full opacity-90 p-1 bg-slate-100">
                            {[...Array(16)].map((_, i) => (
                              <div 
                                key={i} 
                                className={`rounded-[2px] ${
                                  (i % 3 === 0 || i % 5 === 2 || i === 0 || i === 3 || i === 12 || i === 15) ? 'bg-slate-900' : 'bg-transparent'
                                }`} 
                              />
                            ))}
                          </div>
                        ...
                        <div className="space-y-1.5 text-center sm:text-left">
                          <div className="text-[10px] font-black uppercase text-slate-400 tracking-wide">Manual Bind Token</div>
                          <div className="font-mono text-xs font-bold text-slate-800 bg-slate-50 px-2.5 py-1.5 rounded-md border border-slate-100 select-all tracking-wider break-all">
                            CHIB-77AX-66Y9-SEC
                          </div>
                          <p className="text-[9px] text-slate-400">Scan QR Code or enter token manually in Google Authenticator or Duo App.</p>
                        </div>
                      </div>

                      <form onSubmit={handleVerify2FACode} className="space-y-3 pt-2">
                        <div>
                          <label className="block text-[10px] font-black uppercase text-slate-500 tracking-wider mb-1">Enter 6-Digit Verification Code</label>
                          <input
                            type="text"
                            required
                            maxLength={6}
                            ...
                            placeholder="123456"
                            className="w-full sm:max-w-[200px] px-3.5 py-2 bg-slate-50 border border-slate-200 focus:border-[#00c2b2] focus:outline-none rounded-xl text-center font-mono text-lg font-black text-slate-800 tracking-widest"
                          />
                        </div>
                        ...
                        <div className="flex gap-2">
                          <button
                            type="submit"
                            className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-[10px] font-black uppercase tracking-wider transition-colors cursor-pointer"
                          >
                            Verify & Authenticate
                          </button>
                          <button
                            ...
                            className="px-4 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded-xl text-[10px] font-black uppercase tracking-wider transition-colors cursor-pointer"
                          >
                            Cancel
                          </button>
                        </div>
                ...
                {/* 2. Withdrawal Secure PIN */}
                <div className="p-5 bg-slate-50/50 rounded-2xl border border-slate-150 space-y-4">
                  <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                    <div className="flex items-center gap-2">
                      <Key size={18} className="text-purple-600" />
                      <h3 className="text-xs font-black uppercase text-slate-700 tracking-wider">Transaction Withdrawal PIN</h3>
                    </div>
                    <span className="text-[10px] font-mono font-bold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100">
                      PIN: {storedPin === 'Not Configured' ? 'Inactive' : '•••• Enabled'}
                    </span>
                  </div>

                  <p className="text-slate-400 text-[11px] leading-relaxed">
                    A secure 4-digit security code. This cryptographic PIN must be verified on any payout cashout request before funds leave Chibuike corporate vault.
                  </p>

                  {pinSuccessMsg && (
                    <p className="text-[10px] text-emerald-700 bg-emerald-50 py-1.5 px-2.5 border border-emerald-100 font-bold rounded-lg animate-in fade-in">{pinSuccessMsg}</p>
                  )}
                  {pinErrorMsg && (
                    <p className="text-[10px] text-rose-700 bg-rose-50 py-1.5 px-2.5 border border-rose-100 font-bold rounded-lg animate-in fade-in">{pinErrorMsg}</p>
                  )}

                  <form onSubmit={handleSetWithdrawalPinSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-black uppercase text-slate-500 mb-1">New 4-Digit Security PIN</label>
                      <input
                        type="password"
                        required
                        maxLength={4}
                        ...
                        placeholder="••••"
                        className="w-full px-3.5 py-2.5 bg-white border border-slate-200 focus:border-[#00c2b2] focus:outline-none rounded-xl text-xs font-bold font-mono text-center tracking-widest text-[#0a1626]"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] font-black uppercase text-slate-500 mb-1">Confirm PIN Code</label>
                      <input
                        type="password"
                        required
                        maxLength={4}
                    ...
                    <div className="sm:col-span-2">
                      <button
                        type="submit"
                        className="w-full py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-black text-[10px] uppercase tracking-wider rounded-xl cursor-pointer transition-colors"
                      >
                        Activate Cryptographic Payout PIN
                      </button>
                    </div>
                  </form>
                </div>
              ...
              {/* Right Side: Change Password & Session logs */}
              <div className="lg:col-span-5 space-y-8">
                
                {/* 3. Hardened Password Config */}
                <div className="p-5 bg-slate-50/50 rounded-2xl border border-slate-150 space-y-4">
                  <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
                    <Key size={18} className="text-[#00c2b2]" />
                    <h3 className="text-xs font-black uppercase text-slate-700 tracking-wider">Harden Portal Password</h3>
                  </div>

                  {pwSuccessMsg && (
                    <p className="text-[10px] text-emerald-700 bg-emerald-50 py-1.5 px-2.5 border border-emerald-100 font-bold rounded-lg animate-in fade-in">{pwSuccessMsg}</p>
                  )}
                  {pwErrorMsg && (
                    <p className="text-[10px] text-rose-700 bg-rose-50 py-1.5 px-2.5 border border-rose-100 font-bold rounded-lg animate-in fade-in">{pwErrorMsg}</p>
                  )}

                  <form onSubmit={handlePasswordChangeSubmit} className="space-y-3">
                    <div>
                      <label className="block text-[10px] font-black uppercase text-slate-500 mb-1">Current Account Password</label>
                      <input
                        type="password"
                        required
                        value={oldPassword}
                        ...
                        placeholder="••••••••"
                        className="w-full px-3.5 py-2 bg-white border border-slate-200 focus:border-[#00c2b2] focus:outline-none rounded-xl text-xs font-bold text-[#0a1626]"
                      />
                    </div>

                    <div>
                      <label className="block text-[10px] font-black uppercase text-slate-500 mb-1">New System Password</label>
                      <input
                        type="password"
                        required
                        value={newPassword}
                    ...
                    <button
                      type="submit"
                      className="w-full py-2.5 bg-[#00c2b2] hover:bg-[#00a395] text-[#0a1626] font-black text-[10px] uppercase tracking-widest rounded-xl shadow-sm transition-transform cursor-pointer"
                    >
                ...
                <div className="p-5 bg-slate-50/50 rounded-2xl border border-slate-150 space-y-4">
                  <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                    <div className="flex items-center gap-2">
                      <Laptop size={18} className="text-violet-600" />
                      <h3 className="text-xs font-black uppercase text-slate-700 tracking-wider">Active Device Sessions</h3>
                    </div>
                    <span className="text-[9px] text-[#00c2b2] font-black uppercase tracking-wider">
                      {activeSessions.length} Devices
                    </span>
                  </div>

                  <p className="text-slate-400 text-[10px] leading-relaxed">
                    Check or instantly terminate duplicate remote terminal logins to prevent unauthorized backoffice breaches.
                  </p>

                  <div className="space-y-3.5 pt-1">
                    {activeSessions.map((session: any) => (
                      <div key={session.id} className="bg-white p-3 rounded-xl border border-slate-150 flex items-start justify-between gap-3 animate-in fade-in">
                        <div className="space-y-0.5">
                          <p className="text-[11px] font-black text-slate-800 uppercase tracking-tight">{session.device}</p>
                          <p className="text-[9px] text-slate-400 font-semibold">{session.location} ({session.ip})</p>
                          <span className="text-[9px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-md inline-block mt-1 font-mono">{session.time}</span>
                        </div>
                        {session.id !== 'sess-1' && (
                          <button
                            onClick={() => handleRevokeSessionItem(session.id)}
                            className="text-red-500 hover:text-red-700 text-[10px] font-black uppercase tracking-wider py-1 px-2 hover:bg-red-50 rounded-lg transition-colors cursor-pointer shrink-0 border border-slate-100 font-sans"
                          >
                            Revoke
                          </button>
                        )}
                    ...
                    {activeSessions.length === 1 && (
                      <div className="text-center p-3 text-slate-400 text-[10px] font-semibold bg-white border border-dashed border-slate-200 rounded-xl">
                        🔒 No other active session logins detected.
                      </div>
                    )}
                  </div>
                </div>
